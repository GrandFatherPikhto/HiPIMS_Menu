...
// dword_factor
typedef struct s_dword_factor_t {
    int32_t max;
    int32_t min;
    int32_t step;
...
} dword_factor_config_t;

// dword_simple
typedef struct s_dword_simple_t {
    int32_t default_value;
    int32_t step;
    int32_t min;
    int32_t max;

} dword_simple_config_t;
...
typedef struct menu_node_config {
    ...
    uint32_t tag; ///< Static per-node metadata from menu.yaml (e.g. hardware register id), 0 if not set
    ...
    union {        
        ...
        dword_factor_config_t dword_factor;
        dword_simple_config_t dword_simple;
    } data;
} menu_node_config_t;
